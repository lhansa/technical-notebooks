...
compute_interval <- function(size, mean, sd) {
  ...
  error_margin <- qnorm(c(0.025, 0.975)) * sd / sqrt(size)
  confidence_interval <- sample_mean + error_margin
...
```{r}
ggplot(df_confints) + 
  geom_segment(aes(x = experiment, xend = experiment, 
                   y = min_confint, yend = max_confint), 
               color = "#800080") + 
  geom_hline(yintercept = population_mean, color = "yellow", linetype = 2) + 
  labs(x = "Experimento", y = "Intervalo de confianza", 
       title = "Intervalos de confianza de las medias muestrales")
```
